...
def get_iris_data_one_hot(data_master, batch_size=1024):
    '''
    ...
    x = torch.Tensor(x)
    y = make_one_hot(y, num_categories=4)
...
def get_iris_data(in_feat=2, batch_size=1024):
    ...
    d = np.loadtxt("buah.csv")
    x = d[:, :11]
    y = d[:, 11]
    y = np.c_[y]
    tensor_x = torch.Tensor(x) # transform to torch tensor
    tensor_y = torch.Tensor(y) # transform to torch tensor

    td = TensorDataset(tensor_x, tensor_y)
    return DataLoader(td, batch_size=batch_size, shuffle=False)
...
def train_hybrid(data_master, epoch=1):
    '''
    ...
    model = make_anfis(x, num_mfs=3, num_out=4)
...
def train_non_hybrid(in_feat=2):
    ...
    x, y_actual = train_data.dataset.tensors
    model = make_anfis(x, num_mfs=3, num_out=4, hybrid=False)
